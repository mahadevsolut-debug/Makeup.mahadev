<section class="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
    <div class="text-center space-y-4" data-aos="fade-up">
        <span class="text-brand-500 font-bold text-xs uppercase tracking-widest">Pricing & Packages</span>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <?php foreach ($services as $srv): ?>
            <div class="dark-glass-panel rounded-3xl overflow-hidden border border-zinc-800 p-8 flex flex-col justify-between hover:border-brand-500/40 transition" data-aos="fade-up" data-aos-delay="100">
                <div class="space-y-4">
                ...
                <div class="pt-6 mt-6 border-t border-zinc-800 flex items-center justify-between">
                    <span class="text-xs text-zinc-400 block">Starting From <strong class="text-gold-400 text-lg block font-bold"><?= htmlspecialchars($globalSettings['currency_symbol'] ?? '₹') ?><?= number_format($srv['simple_price'] > 0 ? $srv['simple_price'] : 15000) ?></strong></span>
                    <a href="<?= BASE_URL ?>/booking?service=<?= $srv['id'] ?>" class="px-5 py-2.5 rounded-xl bg-brand-500 hover:bg-brand-600 text-white font-bold text-xs transition">
                        Select & Customize
                    </a>
                </div>
            </div>
