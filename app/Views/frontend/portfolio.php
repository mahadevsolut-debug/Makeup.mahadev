<section class="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
    <div class="text-center space-y-4">
        <span class="text-rose-500 font-bold text-xs uppercase tracking-widest">Bridal & Celebrity Portfolio</span>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        <?php foreach ($items as $item): ?>
            <div class="dark-glass-panel rounded-3xl overflow-hidden border border-zinc-800 group hover:scale-[1.02] transition duration-300">
                <div class="h-72 bg-zinc-900 overflow-hidden relative flex items-center justify-center text-zinc-600">
                    ...
                    <span class="absolute bottom-4 left-4 text-xs font-bold px-3 py-1 rounded-full bg-rose-600 text-white">
                ...
                <div class="p-6 space-y-2">
                    ...
                    <?php if ($item['client_name']): ?>
                        <p class="text-xs text-amber-400 font-medium">Client: <?= htmlspecialchars($item['client_name']) ?></p>
                    <?php endif; ?>
                </div>
            </div>
        <?php endforeach; ?>
