<section class="py-16 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="text-center space-y-4 mb-12" data-aos="fade-up">
        <span class="text-brand-500 font-bold text-xs uppercase tracking-widest">Reserve Your Slot</span>
    ...
    <?php if ($bookingSuccess = \App\Core\Session::flash('success_booking')): ?>
        <div class="p-8 rounded-3xl bg-emerald-950/90 border border-emerald-500 text-center space-y-6 mb-12 shadow-2xl" data-aos="zoom-in">
            ...
            <div class="space-y-2">
                ...
                <p class="text-emerald-200 text-sm">Your booking confirmation reference code is <strong class="text-gold-400 font-mono text-base"><?= htmlspecialchars($bookingSuccess['code']) ?></strong>.</p>
            </div>
    ...
    <div x-data="bookingCalculator()" x-init="initCalculator()" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <!-- Booking Form -->
        <div class="lg:col-span-2 dark-glass-panel p-8 rounded-3xl border border-zinc-800 space-y-6" data-aos="fade-right">
            <form action="<?= BASE_URL ?>/booking/submit" method="POST" class="space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="service_id" x-model="selectedServiceId" @change="fetchServiceDetails(true)" class="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-brand-500">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <template x-for="pkg in serviceDetails.packages" :key="pkg.id">
                            <label class="relative border rounded-2xl p-4 cursor-pointer transition flex flex-col justify-between"
                                   :class="selectedPackageId == pkg.id ? 'border-brand-500 bg-brand-900/10' : 'border-zinc-800 bg-zinc-900/50 hover:border-zinc-700'">
                                <div class="space-y-2">
                                    <div class="flex justify-between items-start">
                                        <span class="font-bold text-white text-base" x-text="pkg.name"></span>
                                        <input type="radio" name="package_id" :value="pkg.id" x-model="selectedPackageId" @change="recalculate()" class="text-brand-500 focus:ring-brand-500 bg-zinc-905 border-zinc-705">
                                    </div>
                                    <p class="text-xs text-zinc-400" x-text="pkg.description"></p>
                                </div>
                                <div class="mt-4 pt-2 border-t border-zinc-800/60 font-bold text-gold-400 text-sm">
                        ...
                        <template x-for="addon in serviceDetails.addons" :key="addon.id">
                            <label class="flex items-center justify-between p-4 rounded-2xl border border-zinc-800 bg-zinc-900/40 hover:bg-zinc-900 cursor-pointer transition">
                                <div class="flex items-center gap-3">
                                    <input type="checkbox" name="addons[]" :value="addon.id" x-model="selectedAddons" @change="recalculate()" class="w-4 h-4 text-brand-500 rounded focus:ring-brand-500 bg-zinc-905 border-zinc-705">
                                    <div>
                                ...
                                <span class="text-xs font-bold text-gold-400">+<?= htmlspecialchars($globalSettings['currency_symbol'] ?? '₹') ?><span x-text="Number(addon.price).toLocaleString()"></span></span>
                            </label>
                        </template>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="customer_name" required class="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-brand-500">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-zinc-400 mb-1">Phone / WhatsApp *</label>
                            <input type="tel" name="customer_phone" required class="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-brand-500">
                        </div>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-zinc-400 mb-1">Email Address *</label>
                        <input type="email" name="customer_email" required class="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-brand-500">
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-zinc-400 mb-1">Event Date *</label>
                            <input type="date" name="event_date" required class="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-brand-500">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-zinc-400 mb-1">Ready Time *</label>
                            <input type="time" name="event_time" required class="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-brand-500">
                        </div>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-zinc-400 mb-1">Venue Address / City *</label>
                        <textarea name="location" rows="2" required placeholder="Hotel / Resort / Home address" class="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-brand-500"></textarea>
                    </div>
                </div>

                <button type="submit" class="w-full py-4 rounded-2xl bg-gradient-to-r from-brand-500 to-brand-700 hover:from-brand-600 hover:to-brand-700 text-white font-bold text-base shadow-xl shadow-brand-900/50 transition">
        ...
        <!-- Summary Card Panel -->
        <div class="dark-glass-panel p-6 rounded-3xl border border-zinc-800 h-fit sticky top-28 space-y-6" data-aos="fade-left">
            ...
            <div class="space-y-3 text-sm">
                ...
                <div class="pt-4 border-t border-zinc-800 flex justify-between items-center">
                    ...
                    <span class="text-2xl font-extrabold text-gold-400"><?= htmlspecialchars($globalSettings['currency_symbol'] ?? '₹') ?><span x-text="Number(calculation.total_price).toLocaleString()"></span></span>
                </div>
            </div>

            <div class="p-4 rounded-xl bg-zinc-900 text-xs text-zinc-400 space-y-2">
                <p><i class="fa-solid fa-shield-halved text-brand-500 mr-1"></i> <strong>Zero Hidden Fees:</strong> Pricing includes standard trial session consultation.</p>
            </div>
        </div>
    ...
    return {
        ...
        initCalculator() {
            if (this.selectedServiceId) {
                this.fetchServiceDetails(false);
            }
        },
        async fetchServiceDetails(resetSelections = true) {
            ...
            if (data.status) {
                ...
                if (this.serviceDetails && this.serviceDetails.packages && this.serviceDetails.packages.length > 0 && !this.selectedPackageId) {
                    this.selectedPackageId = this.serviceDetails.packages[0].id;
                    this.recalculate();
                }
            }
        },
